...
function saveToLocalStorage(taskInput, category) {
  const localStorageData = getTasksFromLocalStorage()
  const appropriateTasksList = localStorageData[category]

  appropriateTasksList.unshift(taskInput)
  localStorageData[category] = appropriateTasksList //update the list in tasks object

  localStorage.setItem('tasks', JSON.stringify(localStorageData)) //update the LocalStorage tasks
}
...
function addButtonClick() {
  ...
  if (taskInput && taskInput.trim()) {
    ...
    this.parentElement.querySelector('input').value = '' //clear input box
  } else {
...
function createTaskElement(taskInput) {
  ...
  addDragDropForTask(newTask)
  addRemoveButtonForTask(newTask)

  return newTask
}

//Add a `remove` button for a task
function addRemoveButtonForTask(task) {
...
function handleMultiKeyPress() {
  ...
  addEventListener('keydown', (event) => {
    ...
    if (keys.alt && keys.one) {
      ...
      this.removeEventListener('mouseenter', handleMultiKeyPress)
...
function searchTasks() {
  ...
  const tasksCategories = [
    document.querySelector('.to-do-tasks'),
...
async function saveToBin() {
  ...
  const response = await fetch(
    'https://json-bins.herokuapp.com/bin/614dbbc41f7bafed863ed88f/',
    data
  )
...
async function loadFromBin() {
  ...
  if (response.ok) {
    const tasks = await response
      .json() //convert json file response to object
      .then((dataObject) => dataObject.tasks) //get "tasks" object
      .then(
        document
          .querySelector('.save-load')
...
//Drag & Drop attributes and event listeners for tasks
function addDragDropForTask(task) {
  task.draggable = true
...
document
  ...
  .addEventListener('click', updateTasksFromBin)
